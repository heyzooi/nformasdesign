...
@implementation TwitterXmlParser
...
-(void) parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict {
    if ([elementName isEqualToString:@"item"]) {
        twitterRssItem = [[TwitterRssItem alloc]init];
    }
    [buffer setString:@""];
}
...
-(void) parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName {
    ...
    [buffer replaceOccurrencesOfString:@"&#8221;" withString:@"\"" options:NSCaseInsensitiveSearch range:NSMakeRange(0, [buffer length])];
    if ([elementName isEqualToString:@"item"]) {
        [items addObject:twitterRssItem];
        [twitterRssItem release];
    } else if ([elementName isEqualToString:@"title"]) {
        NSString *title = [buffer copy];
        twitterRssItem.title = title;
        [title release];
    } else if ([elementName isEqualToString:@"pubDate"]) {
        NSString *pubDate = [buffer copy];
        twitterRssItem.pubDate = [dateFormatter dateFromString:pubDate];
        [pubDate release];
    } else if ([elementName isEqualToString:@"link"]) {
        NSString *link = [buffer copy];
        twitterRssItem.link = link;
        [link release];
    } else if ([elementName isEqualToString:@"description"]) {
        NSString *description = [buffer copy];
        twitterRssItem.description = description;
        [description release];
    }
    [buffer setString:@""];
}
